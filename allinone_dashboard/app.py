...
# 2. Load and Preprocess Data (内置 IP 自动解析与缓存刷新逻辑)
@st.cache_data(ttl=86400)
def load_data(_version_hash="v_single_file_001"):
    ...
    if trend_view == "Year-over-Year Comparison":
        ...
        if timebase == "By Month":
            ...
            if not yoy_grouped.empty:
                ...
                st.warning(
                    "Please turn on at least one year and product switch"
                    " above."
                )
        ...
        if not full_grouped.empty:
            ...
            if len(all_periods) == 1:
                ...
                if isinstance(slider_val, (tuple, list)) and len(
                    slider_val
                ) == 2:
...
# ==============================================================================
    ...
    if calc_mode == "All Time Cumulative (历史全量总计)":
        ...
        with col_mode2:
            st.caption(
                "Timeline Slider (Cumulative from start to selected month)"
            )
