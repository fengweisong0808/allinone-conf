...
US_STATE_CENTERS = {
    ...
    "NJ": {"lat": 40.298960, "lon": -74.521011},
...
def load_data(_version_hash="v_single_file_004"):
  ...
  if us_mask.any():
    us_ips = df.loc[us_mask, "PDT_REG_IP"].dropna().unique().tolist()
    ip_to_state_map = {}

    batch_size = 100
    for i in range(0, len(us_ips), batch_size):
      chunk = us_ips[i : i + batch_size]
      clean_chunk = [
    ...
    df["US_State_Code"] = df["PDT_REG_IP"].apply(
        lambda ip: ip_to_state_map.get(str(ip).strip(), "CA")
    )
  else:
    df["US_State_Code"] = "N/A"
...
# ==============================================================================
  ...
  with col_h2:
    ...
    geo_scope = st.radio(
        "Geographic View",
        ["United States (By State)", "Global (By Country Treemap)"],
        horizontal=True,
        label_visibility="collapsed",
    )
  ...
  # 视角 A：美国各州精细热力图（加地图标注数字 + 1:1 置顶悬浮卡片）
  if geo_scope == "United States (By State)":
    ...
    if not state_counts.empty:
      ...
      for _, row in state_counts.iterrows():
        ...
        units_val = row["Sold Units"]
        if st_code in US_STATE_CENTERS and units_val > 0:
          lats.append(US_STATE_CENTERS[st_code]["lat"])
      ...
      fig_map.add_annotation(
          text=(
              ...
              f" style='font-size:20px;'>{total_us_units:,}</span><br><span"
              f" style='font-size:11px; color:gray;'>Period:"
              f" {sel_period}</span>"
          ),
          xref="paper",
          yref="paper",
